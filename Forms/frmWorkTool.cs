...
{
    public partial class frmWorkTool : Form
    {
        ...
        private void BindData()
        {
            ...
            try
            {
                ...
                //////
                slog = DateTime.Now + "[frmWorkTool- 작업횟수] :      " + tgdTotalCnt;
                ...
                //////
                slog = DateTime.Now + "[frmWorkTool TOOL 체결값 체크 시작] :      ";
                Global.EquipInfo.fhLog.TextFileWriteAppend(slog);
                ////// 
            }
            catch (Exception ex)
            ...
            try
            {
                ...
                if (ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    if (rTR_ID == "0" || rTR_ID == "")
                    {
                        ...
                        //////
                        slog = DateTime.Now + "[frmWorkTool TAG MIN/MAX/TR_ID(재전송) 전송] :      " + High_Value + "/" + Low_Value + "/" + EquipInfo.TR_ID;
                        Global.EquipInfo.fhLog.TextFileWriteAppend(slog);
                        //////
                    }
                    else
                    {
                        ...
                        //////
                        slog = DateTime.Now + "[frmWorkTool TAG MIN/MAX 전송] :      " + High_Value + "/" + Low_Value;
                        Global.EquipInfo.fhLog.TextFileWriteAppend(slog);
                        //////
                    }
                }
        ...
        private void Tool_Position()
        {
            try
            {
                ...
                //////
                slog = DateTime.Now + "[frmWorkTool- TOOL 포지션] :      " + EquipInfo.WORKCENTER+"/"+ EquipInfo.BODY_NO.Substring(0,2)+"/"+ EquipInfo.ROUTE_NO+"/"+ EquipInfo.PART_ID+"/"+
        ...
        private Bitmap ResizeImage(string fileName) //이미지를 현재 picturebox 사이즈에 맞도록 수정 20181024 김진성
        {
            ...
            {
                slog = DateTime.Now + "[frmWorkTool ResizeImage] :      " + ex;
                Global.EquipInfo.fh_Err_Log.TextFileWriteAppend(slog);
                return null;
            }
        }
                ...
                if (e.RowIndex == tgdToolOkCnt)
                {
                    ...
                    if (lbl_Number.Text != "")
                    {
                        ...
                        if (double.Parse(lbl_Number.Text) >= double.Parse(Low_Value) && double.Parse(lbl_Number.Text) <= double.Parse(High_Value))
                        {
                            ...
                            datup.Variables.Add("TOOL_ASSY_POINT", new VariableItem { VarID = "TOOL_ASSY_POINT", Value = tgdToolOkCnt  + 1});

                            if (Tool_Value[0].Equals("") && Tool_Value.Length == 1)
                                datup.Variables.Add("TOOL_VALUE", new VariableItem { VarID = "TOOL_VALUE", Value = "Y_" + lbl_Number.Text });
                            else
                            {
                                string strToolValue = "";
                                ...
                                datup.Variables.Add("TOOL_VALUE", new VariableItem { VarID = "TOOL_VALUE", Value = strToolValue + ":Y_" + lbl_Number.Text });
                            }
